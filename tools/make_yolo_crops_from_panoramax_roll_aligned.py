...
改良版:
- crop 後に画像を回転して水平化するのではなく、
  パノラマ全体から推定した pano-level roll を「仮想カメラの roll」として
  透視切り出し時に反映する
- これにより、端部の引き伸ばしや左下/下端の不自然な回転アーティファクトを抑える
- local crop 単体の追加回転は行わない

元ファイル:
- make_yolo_crops_from_panoramax(1).py
"""
...
def equirectangular_to_perspective(
    ...
    # Camera-frame roll first, then pitch, then yaw.
    cos_r, sin_r = math.cos(roll), math.sin(roll)
    x0 = cos_r * x_cam - sin_r * y_cam
    y0 = sin_r * x_cam + cos_r * y_cam
    z0 = z_cam

    cos_p, sin_p = math.cos(pitch), math.sin(pitch)
    x1 = x0
    y1 = cos_p * y0 - sin_p * z0
    z1 = sin_p * y0 + cos_p * z0
...
def main():
    ...
    for i, r in enumerate(index_recs, start=1):
        ...
        if args.level_roll:
            pano_roll_deg, pano_roll_reason, pano_roll_meta = estimate_pano_roll_deg(
                pano,
        ...
        append_jsonl(roll_map_path, {
            ...
            "roll_deg": float(pano_roll_deg),
            "roll_reason": pano_roll_reason,
            "roll_meta": pano_roll_meta,
        })
        ...
        for view_name, yaw_off, fov in views:
            ...
            applied_roll_deg = pano_roll_deg if args.level_roll else 0.0
            ...
            if args.level_roll:
                print(
                    "[roll aligned crop] "
                    f"fid={fid} view={view_name} yaw={yaw:.3f} pano_roll_deg={pano_roll_deg:.3f} "
                    f"roll_reason={pano_roll_reason} final_mode=extract_with_roll"
                )
            ...
            append_jsonl(crop_log_path, {
                ...
                "fov": float(fov),
                "roll_deg": float(applied_roll_deg),
                "roll_reason": pano_roll_reason if args.level_roll else "disabled",
                "roll_meta": {
                    "source": "extract_with_roll",
                    "pano_angle_deg": float(pano_roll_meta.get("angle_deg", 0.0)),
                    "pano_consensus_abs_dev_deg": float(pano_roll_meta.get("consensus_abs_dev_deg", 0.0)),
                    "pano_n_candidates": int(pano_roll_meta.get("n_candidates", 0)),
                },
                "pano_roll_deg": float(pano_roll_deg),
                "pano_roll_reason": pano_roll_reason,
                "pano_roll_meta": pano_roll_meta,
                "local_roll_deg": 0.0,
                "local_roll_reason": "disabled_in_roll_aligned_variant",
                "local_roll_meta": {},
    ...
    summary = {
        ...
        "params": {
            ...
            "api_base": args.api_base,
            "roll_application_mode": "extract_with_roll",
        },
        "paths": {
            "run_dir": str(run_dir),
            "panos": str(panos_dir),
